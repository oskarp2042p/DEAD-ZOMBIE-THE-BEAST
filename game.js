(() => {
  ...
  const QUALITY = {
    low: { detail: 0.45, label: "Low" },
    medium: { detail: 0.75, label: "Medium" },
    high: { detail: 1.50, label: "High" },
    ultra: { detail: 2.50, label: "Ultra" }
  };
  ...
  const UPDATE_LOG = [
    { date: "2026-05-30 12:00", title: "duża aktualizacja", text: "Wiekszy swiat, Stare Miasto, safe zone'y, bossy, misje i backend MVP,Nowy kompas, minimapa jako przycisk, boss bar, dziennik i zakladki ekwipunku Dodano folder mods z darmowymi dodatkami GUI i danymi do przyszlych map." },
    { date: "2026-06-2 18:30", title: "aktualizacja błędów", text: "naprawiono voice zmienio jakość grafiki textzmienino kilka tekstów voice jak i interfejs." },
  ];

  const SHOP_ITEMS = [
    { id: "axe", name: "Siekiera", cost: 67, kind: "weapon", weapon: "axe" },
    { id: "pistol", name: "Pistolet", cost: 120, kind: "weapon", weapon: "pistol" },
    { id: "bow", name: "Luk", cost: 90, kind: "weapon", weapon: "bow" },
    { id: "ammo9", name: "Amunicja 9 mm x12", cost: 32, kind: "item", item: "ammo9", amount: 12 },
    { id: "grenade", name: "Granat x2", cost: 50, kind: "item", item: "grenade", amount: 2 },
    { id: "gui_neon", name: "Darmowy GUI Neon", cost: 0, kind: "addon", addon: "gui-neon" },
    { id: "map_oldtown", name: "Mapa Starego Miasta", cost: 45, kind: "addon", addon: "old-town-map" }
  ];
  ...
  const MISSIONS = [
    { id: "m01", title: "Radio Olivia", desc: "Dotrzyj do panelu pierwszej safe zone.", target: { x: 1090, y: 2235 }, reward: { xp: 40, coins: 12 } },
  ...
  const NOTE_POOL = [
    ...
    { id: "olivia-1", title: "Olivia", text: "Nie kazda safe zone jest domem, ale kazda daje oddech." }
  ];
  ...
  function createGameState() {
    ...
    return {
      ...
      world,
      player,
      olivia: { x: 965, y: 2210, r: 15, facing: 0, talkCooldown: 2 },
  ...
  function loadSavedGame() {
    ...
    try {
      ...
      enterGame();
      speak("Olivia", "Wczytane. Nie zatrzymuj sie, noc tutaj pamieta kazdy blad.");
      return true;
    } catch (error) {
      console.error(error);
  ...
  function migratePlayerShape(player) {
    ...
    player.notes = Array.isArray(player.notes) ? player.notes : ["olivia-1"];
  ...
  function newGame() {
    ...
    enterGame();
    speak("Olivia", "Kyle, slyszysz mnie? Safe zone jest ciemna. Znajdz przewody, baterie i uruchom latarnie UV.");
  ...
  function killBoss(boss) {
    ...
    speak("Olivia", `${boss.name} padl. Baron to poczuje.`);
  ...
  function lootNearby() {
    ...
    if (nearSafe && !isSafeUnlocked(nearSafe.id) && nearSafePanel) {
      ...
      if (consumeItems(p.inventory, cost)) {
        ...
        speak("Olivia", `${nearSafe.name} dziala. Masz lozko, stash, handlarza i latarnie UV.`);
  ...
  function skipDayNight() {
    ...
    speak("Olivia", isNight() ? "Przespales noc. Ulice sa spokojniejsze." : "Czekales do zmroku. Teraz wychodza gorsze rzeczy.");
    saveGame(false);
  }
  ...
  function addXp(amount) {
    ...
    while (player.xp >= needed) {
      ...
      speak("Olivia", `Masz poziom ${player.level}. Wydaj punkt w drzewku umiejetnosci.`);
    }
  }
  ...
  function completeCurrentMission() {
    ...
    speak("Olivia", `Misja zakonczona: ${mission.title}. Nastepny cel: ${currentMission()?.title || "koniec"}.`);
    renderInventoryPanels();
    saveGame(false);
  }
  ...
  function updateOlivia(dt) {
    const olivia = state.olivia;
    const player = state.player;
    const d = distance(olivia, player);
    if (d > 86) {
      const angle = angleTo(olivia, player);
      const speed = d > 260 ? 190 : 118;
      olivia.x += Math.cos(angle) * speed * dt;
      olivia.y += Math.sin(angle) * speed * dt;
      olivia.facing = angle;
    }
  }
  ...
  function hurtPlayer(amount) {
    ...
    if (player.health <= 0) {
      state.gameOver = true;
      speak("Olivia", "Kyle! Nie, trzymaj sie!");
  ...
  function maybeOliviaTalk(dt) {
    const Olivia = state.olivia;
    Olivia.talkCooldown -= dt;
    if (Olivia.talkCooldown > 0) return;

    if (isNight() && Math.random() > 0.55) {
      speak("Olivia", "Po zmroku sa szybsze. Trzymaj latarke UV gotowe.");
      Olivia.talkCooldown = 22;
      return;
    }
    if (isInDarkZone(state.player) && Math.random() > 0.45) {
      speak("Olivia", "To dark zone. Szukaj skrzynek, ale nie gas latarki.");
      Olivia.talkCooldown = 24;
      return;
    }
    if (!isSafeUnlocked("safe") && distance(state.player, { x: 1090, y: 2235 }) < 420) {
      speak("Olivia", "Panel safe zone jest obok bramy. Potrzebujesz zlomu, przewodow i baterii.");
      Olivia.talkCooldown = 26;
    } else {
      Olivia.talkCooldown = 18;
    }
  }
  ...
  function drawOlivia() {
    const Olivia = state.olivia;
    ctx.save();
    ctx.translate(Olivia.x, Olivia.y);
    ctx.rotate(Olivia.facing);
    ctx.fillStyle = "#5f7d8d";
    ctx.beginPath();
    ctx.ellipse(0, 0, Olivia.r * 0.75, Olivia.r, 0, 0, TAU);
    ctx.fill();
    ...
    ctx.font = "12px sans-serif";
    ctx.fillText("Olivia", Olivia.x - 17, Olivia.y - 24);
  }
